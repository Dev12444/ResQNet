...
import { Facility, Incident, Resource } from '@/types';
...
export default function IncidentMap({incidents,resources,facilities,selectedId,onSelect}:{incidents:Incident[];resources:Resource[];facilities:Facility[];selectedId:string|null;onSelect:(i:Incident)=>void}){
 ...
 const rasterStyle = {
  ...
  sources: {
    ...
    hazard: { type: 'geojson', data: { type: 'FeatureCollection', features: [
      {type:'Feature',properties:{color:'#ef4444'},geometry:{type:'Point',coordinates:[72.5521,23.0569]}},
      {type:'Feature',properties:{color:'#f97316'},geometry:{type:'Point',coordinates:[72.6385,22.9684]}},
      {type:'Feature',properties:{color:'#38bdf8'},geometry:{type:'Point',coordinates:[72.585,23.035]}},
      {type:'Feature',properties:{color:'#eab308'},geometry:{type:'Point',coordinates:[72.5087,23.0492]}}
    ]}}
  },
  layers: [
 ...
 return <Map ref={ref} initialViewState={{longitude:72.5714,latitude:23.0225,zoom:11}} mapStyle={rasterStyle} attributionControl={true} style={{width:'100%',height:'100%'}}>
