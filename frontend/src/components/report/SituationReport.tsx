      ...
      <div className="px-5 py-5 sm:px-8 sm:py-7">
        ...
        <header className="border-b-2 border-[var(--foreground)] pb-3">
          <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[var(--muted)]">
            State Disaster Management Authority · ResQNet
          </p>
          ...
          <dl className="mono mt-2.5 grid grid-cols-2 gap-x-6 gap-y-0.5 text-[11px] sm:grid-cols-4">
            <Meta label="Reference" value={doc.id} />
            <Meta label="Period" value={`${date(doc.periodStart)} – ${date(doc.periodEnd)}`} />
            <Meta label="Generated" value={dateTime(doc.generatedAt)} />
            <Meta label="Issued by" value={doc.author} />
          </dl>
        </header>
        ...
        <footer className="mt-8 border-t border-[var(--border)] pt-3">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <p className="mono text-[11px] text-[var(--muted)]">
              Generated by ResQNet · {dateTime(doc.generatedAt)} IST
              <br />
            ...
            <div className="text-right">
              ...
              <p className="mt-1 text-[11px] text-[var(--muted)]">
                {doc.author} — authorised signatory
              </p>
            </div>
          </div>
          <p className="mt-3 text-[10px] leading-snug text-[var(--faint)]">
            Figures in this report are drawn from the ResQNet operational dataset at the
            time of generation. Where a field was unavailable it is marked rather than
            estimated.
          </p>
        </footer>
      </div>
