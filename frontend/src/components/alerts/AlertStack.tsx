...
export function AlertStack({alerts,onAck}:{alerts:Alert[];onAck:(id:string)=>void}){
 ...
 return <div style={{position:'fixed',left:346,bottom:18,width:360,zIndex:20,display:'grid',gap:7}}>
  <div style={{display:'flex',alignItems:'center',gap:7,justifyContent:'flex-end',fontSize:9,color:'var(--muted)'}}>
   {soundBlocked&&!muted&&<span title="Browsers require a click on the page before they will play sound.">SOUND BLOCKED UNTIL YOU CLICK</span>}
   <button onClick={()=>{const next=!muted;setMuted(next);setMutedState(next);}} title={muted?'Unmute alert sound':'Mute alert sound'} style={{display:'flex',alignItems:'center',gap:5,border:'1px solid var(--line-strong)',background:'var(--panel-glass)',color:'var(--muted)',borderRadius:4,height:22,padding:'0 7px',fontSize:8,fontWeight:800,cursor:'pointer'}}>
    {muted?<VolumeX size={11}/>:<Volume2 size={11}/>} {muted?'MUTED':'SOUND ON'}
   </button>
